...
if "result" in st.session_state:
    ...
    if senior_approval is not None:
        ...
        if not st.session_state.get("excel_file") and campaign_store is not None:
            ...
            except (PERSISTENCE_EXCEPTIONS, TypeError, ValueError) as export_error:
        ...
        with st.form(f"senior_review_form_{latest_calendar['id']}"):
            ...
            senior_feedback = st.text_area(
                "Feedback (required when requesting changes)",
                max_chars=5000,
            )
            approve_col, reject_col = st.columns(2)
            with approve_col:
                senior_approve = st.form_submit_button(
                    "Approve Calendar", use_container_width=True
                )
            with reject_col:
                senior_reject = st.form_submit_button(
                    "Request Changes", use_container_width=True
                )

        if senior_approve or senior_reject:
            clean_name = senior_name.strip()
            clean_email = senior_email.strip()
            clean_feedback = senior_feedback.strip()
            if not clean_name or not clean_email:
                st.error("Senior reviewer name and email are required.")
            elif senior_reject and not clean_feedback:
                st.error("Please add feedback before requesting changes.")
            else:
                decision = "approved" if senior_approve else "rejected"
                try:
                    campaign_store.record_approval(
                        campaign_id,
                        latest_calendar["id"],
                        "senior",
                        decision,
